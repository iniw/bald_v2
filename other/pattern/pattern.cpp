...
address pattern::find( address module_base, std::string_view pattern ) {
	...
	static auto pattern_to_byte = [ ]( const char* pattern ) -> std::vector< size_t > {
		...
		for ( auto current = m_start; current < end; ++current ) {
	...
	auto dos_header = reinterpret_cast< PIMAGE_DOS_HEADER >( module_base.as< uintptr_t >( ) );
	if ( dos_header->e_magic != IMAGE_DOS_SIGNATURE )
		return nullptr;

	auto nt_header = reinterpret_cast< PIMAGE_NT_HEADERS >( module_base + dos_header->e_lfanew );
