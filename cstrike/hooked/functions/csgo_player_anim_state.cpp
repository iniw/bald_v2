...
void __vectorcall hooked::update( csgo_player_anim_state* ecx, void* unk0, float z, float y, float x, void* unk1 ) {
	...
	ecx->m_last_update_increment = std::fmaxf( 0.f, g_interfaces.m_globals->m_curtime - ecx->m_last_update_time );

	ecx->m_eye_yaw = g_mathlib_base.angle_normalize( y );
	ecx->m_eye_pitch = g_mathlib_base.angle_normalize( x );
	ecx->m_position_current = player->get_origin( );
	ecx->m_weapon = g_interfaces.m_entity_list->get< weapon_cs_base* >( player->get_active_weapon( ) );
	...
	ecx->m_last_update_time = g_interfaces.m_globals->m_curtime;
...
void __fastcall hooked::modify_eye_position( csgo_player_anim_state* ecx, void* edx, vec_3& input_eye_pos ) {
	...
	if ( head_pos.z < input_eye_pos.z ) {
		...
		float lerp = g_mathlib.simple_spline_remap_val_clamped( std::fabs( input_eye_pos.z - head_pos.z ), 4.f, 10.f, 0.f, 1.f );

		input_eye_pos.z = g_mathlib.lerp( lerp, input_eye_pos.z, head_pos.z );
...
void __fastcall hooked::set_up_velocity( csgo_player_anim_state* ecx, void* edx ) {
	...
	vec_3 abs_velocity = player->get_abs_velocity( );
	...
	ecx->m_velocity = g_mathlib.approach( abs_velocity, ecx->m_velocity, ecx->m_last_update_increment * 2000.f );
	...
	if ( ecx->m_speed_as_portion_of_walk_top_speed >= 1.f )												
		...
		ecx->m_static_approach_speed = g_mathlib.approach( 80.f, ecx->m_static_approach_speed, ecx->m_last_update_increment * 60.f );
	...
	float eye_foot_delta = g_mathlib_base.angle_diff( ecx->m_eye_yaw, ecx->m_foot_yaw );

	float aim_matrix_width_range = g_mathlib.lerp( 
		std::clamp( ecx->m_speed_as_portion_of_walk_top_speed, 0.f, 1.f ), 
		1.f,
		g_mathlib.lerp( ecx->m_walk_to_run_transition, 0.8f, 0.5f ) );

	if ( ecx->m_anim_duck_amount > 0.f )
		aim_matrix_width_range = g_mathlib.lerp( 
	...
	ecx->m_foot_yaw = g_mathlib_base.angle_normalize( ecx->m_foot_yaw );

	if ( ecx->m_velocity_length_xy > 0.1f || std::fabs( ecx->m_velocity_length_z ) > 100.f ) {

		ecx->m_foot_yaw = g_mathlib_base.approach_angle( ecx->m_eye_yaw, ecx->m_foot_yaw, ecx->m_last_update_increment * ( 30.f + 20.f * ecx->m_walk_to_run_transition ) );

		// add server code

	} else {

		ecx->m_foot_yaw = g_mathlib_base.approach_angle( player->get_lower_body_yaw_target( ), ecx->m_foot_yaw, ecx->m_last_update_increment * 100.f );
	...
	if ( ecx->m_velocity_length_xy > 0.f ) {
		...
		ecx->m_move_yaw_ideal = g_mathlib_base.angle_normalize( g_mathlib_base.angle_diff( raw_yaw_ideal, ecx->m_foot_yaw ) );

	}

	ecx->m_move_yaw_current_to_ideal = g_mathlib_base.angle_normalize( g_mathlib_base.angle_diff( ecx->m_move_yaw_ideal, ecx->m_move_yaw ) );
	...
	if ( started_moving_this_frame && ecx->m_move_weight <= 0.f ) {
		...
		if ( animation_layer_movement_move && animation_layer_movement_move->m_sequence != -1.f ) {
			...
			if ( studio_hdr && studio_hdr->seq_desc( move_sequence ).m_anim_tags > 0 ) {

				if ( std::fabsf( g_mathlib_base.angle_diff( ecx->m_move_yaw, 180.f ) ) <= 22.5f )
					ecx->m_primary_cycle = player->get_first_sequence_anim_tag( move_sequence, 1 );
				else if ( std::fabsf( g_mathlib_base.angle_diff( ecx->m_move_yaw, 135.f ) ) <= 22.5f )
					ecx->m_primary_cycle = player->get_first_sequence_anim_tag( move_sequence, 2 );
				else if ( std::fabsf( g_mathlib_base.angle_diff( ecx->m_move_yaw, 90.f ) ) <= 22.5f )
					ecx->m_primary_cycle = player->get_first_sequence_anim_tag( move_sequence, 3 );
				else if ( std::fabsf( g_mathlib_base.angle_diff( ecx->m_move_yaw, 45.f ) ) <= 22.5f )
					ecx->m_primary_cycle = player->get_first_sequence_anim_tag( move_sequence, 4 );
				else if ( std::fabsf( g_mathlib_base.angle_diff( ecx->m_move_yaw, 0.f ) ) <= 22.5f )
		...
		if ( animation_layer_movement_strafechange && animation_layer_movement_strafechange->m_weight >= 1.f ) {
			...
			float move_weight = g_mathlib.lerp( 
				ecx->m_anim_duck_amount,
				std::clamp( ecx->m_speed_as_portion_of_walk_top_speed, 0.f, 1.f ), 
				std::clamp( ecx->m_speed_as_portion_of_crouch_top_speed, 0.f, 1.f ) );

			float ratio = g_mathlib_base.bias( move_weight, 0.18f ) + 0.1f;

			ecx->m_move_yaw = g_mathlib_base.angle_normalize( ecx->m_move_yaw + ( ecx->m_move_yaw_current_to_ideal * ratio ) );
	...
	float aim_yaw = g_mathlib_base.angle_diff( ecx->m_eye_yaw, ecx->m_foot_yaw );
	...
	float pitch = g_mathlib_base.angle_diff( ecx->m_eye_pitch, 0.f );
	if ( pitch > 0.f )
...
void __fastcall hooked::set_up_movement( csgo_player_anim_state* ecx, void* edx ) {
	...
	static auto o_set_up_movement = g_detour.get< decltype( &set_up_movement ) >( "CCSGOPlayerAnimState::SetUpMovement" );
